...
import { Send, Sparkles, Loader2, Brain, Trash2 } from "lucide-react";
...
const HISTORY_KEY = "mente-poderosa:chat:v1";

const SUGGESTIONS = [
  "Como controlar a amígdala antes de uma reunião difícil?",
  "Quais 3 gatilhos de Cialdini funcionam melhor para mulheres?",
  "Como uso o reframing cognitivo numa rejeição amorosa?",
  "Qual o viés de ancoragem aplicado a negociação salarial?",
  "Como aumentar minha janela de tolerância emocional?",
];
...
export default function MentePoderosaChat() {
  ...
  const clearHistory = () => {
    ...
    toast.success("Histórico da IA limpo.");
  };
      ...
      >
        ...
        <div className="flex-1">
          ...
          <p className="text-[11px] text-muted-foreground">
            Especialista em inteligência emocional, psicologia, neurociência e neuromarketing
          </p>
        </div>
          ...
          <div ref={scrollRef} className="max-h-[420px] overflow-y-auto p-4 space-y-3">
            {messages.length === 0 && (
              <div className="space-y-3">
                <p className="text-xs text-muted-foreground">Pergunte qualquer coisa sobre sua mente e influência:</p>
              ...
              <div key={i} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
                <div
                  className={`max-w-[85%] rounded-2xl px-3.5 py-2.5 text-sm ${
                    m.role === "user" ? "bg-gold text-black font-medium" : "bg-muted/40 text-foreground"
                  }`}
                >
                  {m.role === "assistant" ? (
